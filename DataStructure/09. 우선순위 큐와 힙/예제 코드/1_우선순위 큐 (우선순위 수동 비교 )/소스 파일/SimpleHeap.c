#include "SimpleHeap.h"

void HeapInit(Heap *ph)
{
    ph->numOfData = 0;
}

int HIsEmpty(Heap *ph)
{ // 힙이 비었는지 확인
    if (ph->numOfData == 0)
        return TRUE;
    else
        return FALSE;
}

int GetParentIdx(int idx)
{ // 부모 노드의 인덱스 값 반환
    return idx / 2;
}

int GetLChildIdx(int idx)
{ // 왼쪽 자식 노드의 인덱스 값 반환
    return idx * 2;
}

int GetRChildIdx(int idx)
{ // 오른쪽 자식 노드의 인덱스 값 반환
    return GetLChildIdx(idx) + 1;
}

int GetHighPriChildIdx(Heap *ph, int idx)
{ // 두 개의 자식 노드 중 높은 우선순위의 자식 노드 인덱스 값 반환
    int LChildIdx = GetLChildIdx(idx);
    int RChildIdx = GetRChildIdx(idx);

    if (LChildIdx > ph->numOfData) // 데이터 수보다 인덱스 값이 크다면, 존재하지 않는 데이터이기 때문에 0 반환 (즉, 자식 노드가 없는 경우)
        return 0;
    else if (LChildIdx == ph->numOfData) // 데이터 수와 왼쪽 데이터 인덱스 값이 같다면, 마지막 데이터이므로 바로 반환해준다. (즉, 왼쪽 자식 노드 하나만 존재하는 경우)
        return LChildIdx;
    else
    { // 왼쪽 오른쪽 자식 노드가 모두 존재하는 경우
        if (ph->heapArr[LChildIdx].pr > ph->heapArr[RChildIdx].pr)
            return RChildIdx;
        else
            return LChildIdx;
    }
}

void HInsert(Heap *ph, HData data, Priority pr)
{
    int idx = ph->numOfData + 1; // 새 노드가 저장될 인덱스 값을 idx에 저장, 새로운 데이터는 우선 순위가 제일 낮다는 가정하에 초기화
    HeapElem nelem = {pr, data}; // 새 노드의 생성 및 초기화

    // 새 노드가 저장될 위치가 루트 노드의 위치가 아니라면 while문 반복
    while (idx != 1)
    {
        // 새 노드와 부모 노드의 우선순위 비교
        if (pr < (ph->heapArr[GetParentIdx(idx)].pr))
        {                                                      // 우선 순위 값이 낮으면, 더 높은 우선 순위이므로...
            ph->heapArr[idx] = ph->heapArr[GetParentIdx(idx)]; // 부모 노드를 한 레벨 내림
            idx = GetParentIdx(idx);                           // 새 노드를 한 레벨 올림
        }
        else
            break;
    }

    ph->heapArr[idx] = nelem; // 새 노드를 배열에 저장
    ph->numOfData += 1;
}

HData HDelete(Heap *ph)
{
    HData retData = (ph->heapArr[1]).data;          // 반환을 위해서 삭제할 데이터 저장
    HeapElem lastElem = ph->heapArr[ph->numOfData]; // 힙의 마지막 노드 저장

    // 아래의 변수 parentIdx에는 최종적으로 마지막 노드가 저장될 위치정보가 담긴다.
    int parentIdx = 1; // 루트 노드가 위치해야할 인덱스 값 저장
    int childIdx;

    // 루트 노드의 우선순위가 높은 자식 노드를 시작으로 반복문 시작
    while (childIdx = GetHighPriChildIdx(ph, parentIdx))
    {
        if (lastElem.pr <= ph->heapArr[childIdx].pr) // 마지막 노드와 우선순위 비교
            break;                                   // 마지막 노드의 우선 순위가 높으면 반복문 탈출

        ph->heapArr[parentIdx] = ph->heapArr[childIdx]; // 마지막 노드보다 우선순위가 높으니, 비교 대상 노드의 위치를 한 레벨 올림
        parentIdx = childIdx;                           // 마지막 노드가 저장될 위치 정보를 한 레벨 내림
    }

    ph->heapArr[parentIdx] = lastElem; // 마지막 노드 최종 저장
    ph->numOfData -= 1;
    return retData;
}
